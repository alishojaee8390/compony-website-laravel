...
@section('main-content')
    <section class="p-3 rounded-3 bg-white-20">
        ...
        <div>
            <form action="{{ route('blog.update', $blog->id) }}" method="post" enctype="multipart/form-data" id="form">
                @method('PUT')
                @csrf
                <div class="row mt-3">
                    ...
                    <div class="col-12 col-md-6 mt-3 p-2">
                        ...
                        <input type="text" class="form-control p-1 mb-2" id="subject" name="subject"
                            placeholder="عنوان بلاگ را وارد کنید" value="{{ old('subject' , $blog->subject) }}">
                        @error('subject')
                    ...
                    <div class="col-12 col-md-6 mt-3 p-2">
                        ...
                        @foreach ($categories as $category)
                                <option value="{{$category->id}}" @if (old('category_id' , $category->id) === $blog->category_id)
                                    selected
                    ...
                    <div class="col-12 col-md-6 mt-3 p-2">
                        ...
                        placeholder="آلت بلاگ را وارد کنید" value="{{ old('alt' , $blog->alt) }}">
                        @error('alt')
                        <span class="text-danger mt-3">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="col-12 mt-3 p-2">
                        ...
                        <textarea name="body" id="body" cols="30" rows="30">{{old('body' , $blog->body)}}</textarea>
                        @error('body')
                            <span class="text-danger mt-3">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="col-12 col-md-6 mt-3 p-2">
                        ...
                        <input type="text" class="form-control p-1 mb-2" id="title" name="title"
                            placeholder="عنوان صفحه را وارد کنید" value="{{ old('title' , $blog->title) }}">
                        @error('title')
                            <span class="text-danger mt-3">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="col-12 col-md-6 mt-3 p-2">
                        ...
                        <input type="text" class="form-control p-1 mb-2" id="description" name="description"
                            placeholder="درباره صفحه را وارد کنید" value="{{ old('description' , $blog->description) }}">
                        @error('description')
                            <span class="text-danger mt-3">{{ $message }}</span>
                        @enderror
                    </div>
