...
@section('main-content')
    <section class="p-3 rounded-3 bg-white-20">
        ...
        <div
            ...
            <div>
                <a href="{{ route('faq.create') }}" class="btn btn-success btn-sm p-2">ایجاد سوال</a>
            </div>
            <div>
                ...
                <tbody>
                    @foreach ($faqs as $key => $faq)
                        <tr>
                            ...
                            <td class="text-center">{{ Str::limit(strip_tags($faq->answer) , 30) }}</td>
                            <td class="d-flex gap-2 text-start justify-content-end w-16-rem align-items-center">
                                ...
                                <form action="{{ route('faq.destroy', $faq->id) }}" class="mb-2" method="POST">
                                    @method('DELETE')
                                    @csrf
