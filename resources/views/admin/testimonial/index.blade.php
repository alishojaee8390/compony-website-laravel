...
@section('main-content')
    <section class="p-3 rounded-3 bg-white-20">
        ...
        <div
            ...
            <div>
                <a href="{{ route('testimonial.create') }}" class="btn btn-success btn-sm p-2">ایجاد نظر</a>
            </div>
            <div>
                ...
                <tbody>
                    @foreach ($testimonials as $key => $testimonial)
                        <tr>
                            ...
                            <td class="text-center">{{ $testimonial->fullName }}</td>
                            <td class="text-center">{{ $testimonial->job }}</td>
                            <td class="text-center">{{ $testimonial->description }}</td>
                            <td class="text-center img-fluid"><img src="{{asset('images/testimonial/'.$testimonial->image)}}" style="width: 45px; height: 45px;" alt="{{$testimonial->alt}}"></td>
                            <td class="text-center">{{ $testimonial->alt }}</td>
                            <td class="d-flex gap-2 text-start justify-content-end w-16-rem align-items-center">
                                ...
                                <form action="{{ route('testimonial.destroy', $testimonial->id) }}" class="mb-2" method="POST">
                                    @method('DELETE')
                                    @csrf
