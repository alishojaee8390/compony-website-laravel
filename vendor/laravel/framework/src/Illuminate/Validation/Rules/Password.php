    ...
    public function passes($attribute, $value)
    {
        ...
        $validator = Validator::make(
            ...
            if ($this->mixedCase && ! preg_match('/(\p{Ll}+.*\p{Lu})|(\p{Lu}+.*\p{Ll})/u', $value)) {
                $validator->errors()->add($attribute, 'The :attribute باید شامل حروف بزرگ و کوچک باشد.');
            }

            if ($this->letters && ! preg_match('/\pL/u', $value)) {
                $validator->errors()->add($attribute, 'The :attribute باید شامل یک حرف باشد.');
            }

            if ($this->symbols && ! preg_match('/\p{Z}|\p{S}|\p{P}/u', $value)) {
                $validator->errors()->add($attribute, 'The :attribute باید شامل کارکتر خاص باشد.');
            }

            if ($this->numbers && ! preg_match('/\pN/u', $value)) {
                $validator->errors()->add($attribute, 'The :attribute باید شامل عدد باشد.');
            }
        });
        ...
        if ($this->uncompromised && ! Container::getInstance()->make(UncompromisedVerifier::class)->verify([
            ...
            return $this->fail(
                'The given :attribute این پسورد لو رفته است پسورد دیگری وارد کنید :attribute.'
            );
        }
